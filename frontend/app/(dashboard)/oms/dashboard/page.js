...
const Dashboard = () => {
    ...
    try {
      ...
      if (isEdit) {
        setEditOrder({
          ...
          driver_details: fullOrder.driver_details ? (fullOrder.driver_details) : { name: '', vehicle_number_plate: '', vehicle_type: '', max_capacity: '' },
  ...
  const handleRedownloadDocument = async (orderId, type) => {
    ...
    try {
      ...
      const savedBagWeight = order.bag_weight_kg;

      if (!isValidBagWeight(savedBagWeight)) {
        setSnackbar({
          open: true,
          message: 'No saved bag weight for this order. Generate the document again from the order actions first.',
          severity: 'warning',
        });
        return;
      }
      ...
      if (type === 'DO') {
        ...
        setDeliveryDate(dayjs().format('YYYY-MM-DD'));
        setBagWeightKg(String(savedBagWeight));
        ...
        generateSPKPDF(pdfPayload, savedBagWeight).save(`SPK_${paddedId}_${dateSuffix}.pdf`);
        setSnackbar({ open: true, message: 'SPK regenerated successfully', severity: 'success' });
      } else if (type === 'SPM') {
        generateSPMPDF(pdfPayload, savedBagWeight).save(`SPM_${paddedId}_${dateSuffix}.pdf`);
                      ...
                      {selectedOrder.shipping_method === 'Customer' && selectedOrder.driver_details && (
                        <>
                          <TextField
                            fullWidth
                            label="Driver Name"
                            value={(selectedOrder.driver_details)?.name || '-'}
                            InputProps={{ readOnly: true }}
                            sx={{ mb: 2 }}
                          />
                          <TextField
                            fullWidth
                            label="Vehicle Number Plate"
                            value={(selectedOrder.driver_details)?.vehicle_number_plate || '-'}
                            InputProps={{ readOnly: true }}
                            sx={{ mb: 2 }}
                          />
                          <TextField
                            fullWidth
                            label="Vehicle Type"
                            value={(selectedOrder.driver_details)?.vehicle_type || '-'}
                            InputProps={{ readOnly: true }}
                            sx={{ mb: 2 }}
                          />
                          <TextField
                            fullWidth
                            label="Max Capacity (kg)"
                            value={(selectedOrder.driver_details)?.max_capacity || '-'}
                            InputProps={{ readOnly: true }}
                            sx={{ mb: 2 }}
                          />
                        </>
