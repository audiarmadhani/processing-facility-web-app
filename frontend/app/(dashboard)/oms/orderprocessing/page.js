...
import { bagsForItem } from '../_shared/bagWeightUtils';
...
// --- helper: refreshOrder and updateOrderStatus (ensure UI sees server timestamps) ---
const refreshOrder = async (orderId) => {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'https://processing-facility-backend.onrender.com'}/api/orders/${orderId}`, {
...
const updateOrderStatus = async (orderId, putBody) => {
  try {
    const putRes = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'https://processing-facility-backend.onrender.com'}/api/orders/${orderId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
      body: JSON.stringify(putBody),
    });
    if (!putRes.ok) throw new Error('Failed to update order: ' + (await putRes.text()));
    // fetch authoritative copy
...
useEffect(() => {
    ...
    try {
      ...
      const processingUpdateRes = await fetch(`https://processing-facility-backend.onrender.com/api/orders/${orderId}`, {
        ...
        body: JSON.stringify({
          ...
          status: 'Processing',
          driver_id: order?.driver_id || null || null,
          shipping_method: order.shipping_method || 'Self',
          driver_name: (selectedOrder?.driver_name || order?.driver_name || ''),
          driver_vehicle_number: (selectedOrder?.driver_vehicle_number || order?.driver_vehicle_number || ''),
          driver_vehicle_type: (selectedOrder?.driver_vehicle_type || order?.driver_vehicle_type || ''),
    ...
    try {
      ...
      const readyUpdateRes = await fetch(`https://processing-facility-backend.onrender.com/api/orders/${orderId}`, {
        ...
        body: JSON.stringify({
          ...
          driver_id: order?.driver_id || null, // Reuse existing driver_id
          shipping_method: order.shipping_method || 'Self', // Default to 'Self' if missing
          driver_name: (selectedOrder?.driver_name || order?.driver_name || ''),
          driver_vehicle_number: (selectedOrder?.driver_vehicle_number || order?.driver_vehicle_number || ''),
          driver_vehicle_type: (selectedOrder?.driver_vehicle_type || order?.driver_vehicle_type || ''),
          driver_max_capacity: (selectedOrder?.driver_max_capacity || order?.driver_max_capacity || ''),
    ...
    try {
      ...
      const transitUpdateRes = await fetch(`https://processing-facility-backend.onrender.com/api/orders/${orderId}`, {
        ...
        body: JSON.stringify({
          ...
          price: order.price?.toString() || '0', // Reuse existing price, converted to string
          tax_percentage: order.tax_percentage?.toString() || '0', // Reuse existing tax_percentage, converted to string
          // Status-only update — leave existing line items untouched
          shipping_address: order.shipping_address || order.customer_address,
          billing_address: order.billing_address || order.shipping_address || order.customer_address,
        }),
      });
  ...
  const handleDownloadDocuments = () => {
    ...
    try {
      ...
      generateSPKPDF(pdfPayload).save(`SPK_${String(order.order_id).padStart(4, '0')}_${dateSuffix}.pdf`);
      generateSPMPDF(pdfPayload).save(`SPM_${String(order.order_id).padStart(4, '0')}_${dateSuffix}.pdf`);
    ...
    try {
      ...
      const orderUpdateRes = await fetch(`https://processing-facility-backend.onrender.com/api/orders/${orderId}`, {
        ...
        body: JSON.stringify({
          ...
          driver_id: selectedOrder?.driver_id || null, // Reuse existing driver_id
                  ...
                  {(selectedOrder.items || []).map((item, index) => (
                    <Box key={index} sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                      ...
                      <TextField
                        label="Bags"
                        type="number"
                        value={item.jumlah_karung ?? bagsForItem(item) ?? ''}
                        sx={{ mr: 2, width: 90 }}
                        InputProps={{ readOnly: true }}
                      />
                      <TextField
