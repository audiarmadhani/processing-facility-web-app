...
const ArabicaWeightMoM = ({ timeframe = "this_month" }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(response.data.arabicaWeightMoM)) {
          const formattedData = response.data.arabicaWeightMoM.map(item => ({
            date: item.Date,
        ...
        } else if (response.data.arabicaWeightMoM) { // Handle single object case
          const formattedData = [{
            date: response.data.arabicaWeightMoM.Date,
            thisMonth: parseFloat(response.data.arabicaWeightMoM.TotalWeightThisMonth),
            lastMonth: parseFloat(response.data.arabicaWeightMoM.TotalWeightLastMonth),
          }];
          setData(formattedData);
        } else {
          console.error("Invalid data format:", response.data.arabicaWeightMoM);
...
export default ArabicaWeightMoM;