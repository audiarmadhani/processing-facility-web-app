...
const RobustaProcessedMoM = ({ timeframe = "this_month" }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(response.data.robustaProcessedMoM)) {
        ...
        } else if (response.data.robustaProcessedMoM) { // Handle single object case
          const formattedData = [{
            date: response.data.robustaProcessedMoM.Date,
            thisMonth: parseFloat(response.data.robustaProcessedMoM.TotalWeightThisMonth),
            lastMonth: parseFloat(response.data.robustaProcessedMoM.TotalWeightLastMonth),
          }];
          setData(formattedData);
        } else {
          console.error("Invalid data format:", response.data.robustaProcessedMoM);
