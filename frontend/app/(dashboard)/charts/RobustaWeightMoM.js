  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        } else if (response.data.robustaWeightMoM) { // Handle single object case
          const formattedData = [{
            date: response.data.robustaWeightMoM.Date,
            thisMonth: parseFloat(response.data.robustaWeightMoM.TotalWeightThisMonth),
            lastMonth: parseFloat(response.data.robustaWeightMoM.TotalWeightLastMonth),
          }];
          setData(formattedData);
        } else {
          console.error("Invalid data format:", response.data.robustaWeightMoM);
...
export default RobustaWeightMoM;