...
const ArabicaProcessedMoM = ({ timeframe = "this_month" }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(response.data.arabicaProcessedMoM)) {
          const formattedData = response.data.arabicaProcessedMoM.map(item => ({
            date: item.Date,
        ...
        } else if (response.data.arabicaProcessedMoM) { // Handle single object case
          const formattedData = [{
            date: response.data.arabicaProcessedMoM.Date,
            thisMonth: parseFloat(response.data.arabicaProcessedMoM.TotalWeightThisMonth),
            lastMonth: parseFloat(response.data.arabicaProcessedMoM.TotalWeightLastMonth),
          }];
          setData(formattedData);
        } else {
          console.error("Invalid data format:", response.data.arabicaProcessedMoM);
...
export default ArabicaProcessedMoM;