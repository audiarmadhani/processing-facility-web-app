...
router.get('/qc', async (req, res) => {
    try {
      ...
      const [allRows] = await sequelize.query('SELECT a.*, DATE("qcDate") "qcDateTrunc" FROM "QCData" a');
  
      // Fetch the latest records ordered by QC date
      const [latestRows] = await sequelize.query('SELECT a.*, DATE("qcDate") "qcDateTrunc" FROM "QCData" a ORDER BY a."qcDate" DESC LIMIT 1');
