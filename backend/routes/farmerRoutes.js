...
router.post('/farmer', async (req, res) => {
  ...
  try {
    const { farmerName, farmerAddress, bankAccount, bankName, farmerLandArea, farmerContact, latitude, longitude, farmType, notes, farmVarieties } = req.body;

    // Save the farmer data
    const [farmerData] = await sequelize.query(
      'INSERT INTO "Farmers" ("farmerName", "farmerAddress", "bankAccount", "bankName", "farmerLandArea", "farmerContact", "latitude", "longitude", "farmType", "notes", "farmVarieties") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      {
        replacements: [farmerName, farmerAddress, bankAccount, bankName, farmerLandArea, farmerContact, latitude, longitude, farmType, notes, farmVarieties],
        transaction: t,
      }
    );
