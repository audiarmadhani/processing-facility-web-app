-- Import dry mill entries + huller weights from Sheet4.csv (216 batches)
-- Run in Supabase SQL editor. Review preview queries before committing.
-- Sets status to "In Dry Mill" via DryMillData (rfid + entered_at from DryingData.exited_at).
-- Inserts huller events only when none exist yet (does not overwrite existing weights).
--
...
-- ---------------------------------------------------------------------------
-- 1) Staging data from spreadsheet
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2) PREVIEW — run this section alone first if you want to inspect matches
-- ---------------------------------------------------------------------------
-- SELECT
--   s.batch_number,
--   pp."processingType",
...
-- ---------------------------------------------------------------------------
-- 3) Enriched rows (preprocessing + receiving + drying exit + rfid)
--    entered_at = latest DryingData.exited_at (required; no synthetic fallback)
-- ---------------------------------------------------------------------------
CREATE TEMP TABLE dry_mill_import_enriched AS
SELECT
  s.batch_number,
  s.input_weight,
  s.hulled_weight,
  pp."processingType",
  COALESCE(pp."producer", r."producer", 'N/A') AS producer,
  d.exited_at AS entered_at,
  COALESCE(d.rfid, r.rfid) AS rfid
FROM dry_mill_import_staging s
INNER JOIN LATERAL (
  SELECT pp2."processingType", pp2."producer"
  FROM "PreprocessingData" pp2
  WHERE pp2."batchNumber" = s.batch_number
  ORDER BY pp2."createdAt" DESC NULLS LAST, pp2.id DESC
  LIMIT 1
...
INNER JOIN LATERAL (
  SELECT d2.exited_at, d2.rfid
  FROM "DryingData" d2
  WHERE d2."batchNumber" = s.batch_number
    AND d2.exited_at IS NOT NULL
  ORDER BY d2.exited_at DESC NULLS LAST, d2.id DESC
  LIMIT 1
) d ON true
LEFT JOIN "ReceivingData" r ON r."batchNumber" = s.batch_number
WHERE s.hulled_weight IS NOT NULL
  AND s.hulled_weight > 0
  AND COALESCE(d.rfid, r.rfid) IS NOT NULL;

-- ---------------------------------------------------------------------------
-- 4) DryMillData — enter batches (status -> In Dry Mill)
-- ---------------------------------------------------------------------------
...
SELECT
  ...
  NOW(),
  NOW()
FROM dry_mill_import_enriched e
WHERE NOT EXISTS (
  SELECT 1
  FROM "DryMillData" dm
  WHERE dm."batchNumber" = e.batch_number
    AND dm."processingType" = e."processingType"
    AND dm.entered_at IS NOT NULL
    AND dm.exited_at IS NULL
);

UPDATE "DryMillData" dm
SET
  rfid = COALESCE(dm.rfid, e.rfid),
  entered_at = e.entered_at,
  "updatedAt" = NOW()
FROM dry_mill_import_enriched e
WHERE dm."batchNumber" = e.batch_number
  AND dm."processingType" = e."processingType"
  AND (dm.entered_at IS NULL OR dm.exited_at IS NOT NULL);

-- ---------------------------------------------------------------------------
-- 5) DryMillProcessEvents — huller step (insert only if not already recorded)
-- ---------------------------------------------------------------------------
...
SELECT
  ...
  e.producer,
  NULL,
  COALESCE(e.input_weight, 0),
  e.hulled_weight,
  'Sheet4 import',
  'Imported from Sheet4.csv',
  1,
  NOW(),
  NOW()
FROM dry_mill_import_enriched e
WHERE NOT EXISTS (
  SELECT 1
  FROM "DryMillProcessEvents" pe
  WHERE pe."batchNumber" = e.batch_number
    AND pe."processingType" = e."processingType"
    AND pe."processStep" = 'huller'
);
...
SELECT
  ...
  dm.exited_at,
  dm."processingType",
  e."outputWeight" AS huller_output_kg,
  CASE
    WHEN dm.entered_at IS NOT NULL AND dm.exited_at IS NULL THEN 'In Dry Mill'
    WHEN dm.entered_at IS NULL THEN 'Not Started'
    ELSE 'Processed / Exited'
  END AS dry_mill_status
FROM dry_mill_import_staging s
LEFT JOIN dry_mill_import_enriched en ON en.batch_number = s.batch_number
LEFT JOIN "DryMillData" dm
  ON dm."batchNumber" = s.batch_number
 AND dm."processingType" = en."processingType"
LEFT JOIN "DryMillProcessEvents" e
  ON e."batchNumber" = s.batch_number
 AND e."processingType" = en."processingType"
 AND e."processStep" = 'huller'
ORDER BY s.batch_number;

-- Batches in CSV but skipped
SELECT
  s.batch_number,
  s.input_weight,
  s.hulled_weight,
  CASE
    WHEN pp."batchNumber" IS NULL THEN 'NO PreprocessingData match'
    WHEN d."batchNumber" IS NULL OR d.exited_at IS NULL THEN 'NO DryingData.exited_at'
    WHEN COALESCE(d.rfid, r.rfid) IS NULL THEN 'NO rfid on DryingData or ReceivingData'
    WHEN s.hulled_weight IS NULL OR s.hulled_weight <= 0 THEN 'INVALID hulled weight'
    ELSE 'UNKNOWN'
  END AS reason
FROM dry_mill_import_staging s
LEFT JOIN dry_mill_import_enriched e ON e.batch_number = s.batch_number
LEFT JOIN LATERAL (
...
LEFT JOIN "ReceivingData" r ON r."batchNumber" = s.batch_number
WHERE e.batch_number IS NULL
ORDER BY reason, s.batch_number;
